...
| CONN-006 | Connector |   WARN   |          Port Availability Check           | Checks whether another process listens on a configured port, read from `/proc/net/tcp` without binding anything. While this Tomcat runs, only a listener owned by a different process is reported. Skipped where `/proc` is unavailable. |       Process State        | Stop the process using the port, or change the Tomcat port in `server.xml`.                                                            |
